...
class CellularAutomaton {
   ...
   public:
    ...
    //getRows returns rows for other classes
    //parameters: none
    //Returns: int
    int getRows()
    {
    ...
    //getCols returns Cols for other classes
    //parameters: none
    //Returns: int
    int getCols()
    {
    ...
    //for loading a shape from jason file to the grid
    //parameters: int, int, int
    //returns: void
    void setCellShape(int row, int col, int value) 
    {
        if (row >= 0 && row < rows && col >= 0 && col < cols) 
        {
            grid[row][col] = value;
    ...
    //function for clearing the grid when called
    //parameters: none
    //returns: void
    void clearGrid()
    {
